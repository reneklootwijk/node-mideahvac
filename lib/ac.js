...
function reportProperties(properties) {
  ...
  for (let property in properties) {
    switch (property) {
      ...
      case 'cosySleepMode':
        let modes = ['no sleep', 'sleep 1', 'sleep 2', 'sleep 3']
...
module.exports = class extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      ...
      self._request(cmd, 'getStatus', retry)
        .then(response => {
          ...
          let updates = self._updateStatus(parsedData)        
  ...
  setStatus(properties = {}, retry = false, emitUpdates = true) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // H: powerOn  
      status.keyStatus = true
      status.remoteControlMode = true
      cmd[1] = (status.keyStatus ? 0x40 : 0x00) | (status.fastCheckActive ? 0x20 : 0x00) |
      ...
      // H: wiseEye
      cmd[9] = (status.ecoModeActive ? 0x80 : 0x00) | (status.purifyingMode ? 0x40 : 0x00) |
