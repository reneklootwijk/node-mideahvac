...
module.exports = class extends EventEmitter {
  ...
  _authenticate () {
    ...
    if (self._loginInProgress === null) {
      ...
      self._loginInProgress = new Promise(async (resolve, reject) => {
        ...
        try {
          self._loginId = await self._getLoginId(self.uid)
        } catch (error) {
          return reject(error)
        }
  ...
  _createKey () {
    ...
    const key = self._decrypt(self._accessToken, keyHash)
  ...
  _decrypt (data, key) {
    ...
    blocks.forEach(block => {
      ...
      const decrypted = decipher.update(block, 'hex') + decipher.final('hex')
  ...
  _encrypt (data, key) {
    ...
    blocks.forEach(block => {
      ...
      const encrypted = cipher.update(block, 'hex', 'hex') + cipher.final('hex')
  ...
  request (cmd, label = 'unknown', deviceId, retry = false) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const packet = createPacket(cmd, self._messageId, deviceId)
      ...
      var sendRequest = () => {
        self._apiRequest({ method: 'POST', path: '/v1/appliance/transparent/send' }, {
          ...
          .then(response => {
            ...
            const decrypted = self._decrypt(response.result.reply)

            // Decode the response
            const data = Buffer.from(Buffer.from(decrypted).toString().split(','))
