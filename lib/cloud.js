...
module.exports = class {
    ...
    return new Promise((resolve, reject) => {
      ...
      const retry = () => {
        const req = https.request(options, res => {
          res.on('data', data => {
            logger.debug(`MideaCloudClient._apiRequest: Response ${data.toString()}`);
  ...
  _getLoginId () {
    ...
    logger.silly(`MideaCloudClient._getLoginId: Entering with uid = ${self.uid}`);
  ...
  async getToken (udpId) {
    ...
    logger.silly(`MideaCloudClient._getToken: Entering with udpId = ${udpId}`);
