...
module.exports = class extends AC {
      ...
      self._connection.on('data', function (data) {
        ...
        if (data[0] === 0xAA) {
          ...
          if (self._cmdInProgress) {
            logger.debug(`SerialBridge.connect: Calling handler for the command '${self._cmdQueue[0].label}' in progress`)
  ...
  _processQueue () {
    ...
    if (self._cmdInProgress) {
      return logger.debug(`SerialBridge._processQueue: Command in progress`)
    }

    if (!self._cmdQueue.length) {
      return logger.debug(`SerialBridge._processQueue: No queued commands`)
    }
  ...
  async initialize() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        logger.debug(`SerialBridge.initialize: Current status of ${self._deviceId} - ${JSON.stringify(status)}`)
      } catch(error) {
      ...
      try {
        ...
        logger.debug(`SerialBridge.initialize: Capabilities of ${self._deviceId} - ${JSON.stringify(capabilities)}`)
      } catch(error) {
